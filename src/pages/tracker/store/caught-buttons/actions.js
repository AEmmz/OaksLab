...
import { catchLock } from '../../../../util/tracker/catchLock.js';
...
export default {
	async toggler({ commit, state, rootGetters }, payload) {
		try {
			...
			const updateState = await commit('toggler', type);
			...
			const updateDb = await update(dbRef, dbSelector);
		} catch (error) {
	...
	async caughtCheck({ commit, dispatch, rootGetters }, payload) {
		try {
			...
			if (catchData === null) {
				...
				const updateDb = await update(newDbRef, {
					catch: catchData,
					name: setName,
					dexNo: dexNo,
				});
	...
	async lockCheck({ commit, rootGetters }) {
		try {
			const pkId = rootGetters['tracker/pkId'];
			const pkIdVar = rootGetters['tracker/pkId'];
